...
$valorobra = 3;
$nome = $_POST['nomenovela'];
$diretor = $_POST['diretornovela'];
$ano = $_POST['anonovela'];
$elenco = $_POST['elenconovela'];
$sinopse = $_POST['sinopsenovela'];
$episodios = $_POST['epsnovela'];
$emissora = $_POST['emissora'];
$classif = $_POST['classifnovela'];
$foto = $_FILES['fotonovela']['name'];
$foto2 = $_FILES['fotonovela']['tmp_name'];

$fotoNomeInput = 'fotonovela'; // Nome do input do formulário usado para enviar o arquivo
$diretorioFotoN = "../imagens/img_obras/";

if ($_SERVER["REQUEST_METHOD"] == "POST") {

//Verificando se novela já está no bd
...
if(@move_uploaded_file($foto2, $diretorioFotoN.$foto))
{
    //Verificando se tem algum campo em branco 
    if (empty($nome) || empty($diretor) || empty($ano) || empty($sinopse) || empty($foto) || empty($elenco) || empty($episodios) || empty($emissora)) {
     ...
     if ($resultado = $emailDoCliente) {
         ...
         try {
             ...
             $mail->Body = "<form action='obra_permitida.php' name='verificar' method='POST'>
                            Nome da novela: $nome <br> 
                            Diretor: $diretor <br> 
                            Ano de lançamento: $ano <br> 
                            Sinopse: $sinopse <br>
                            Elenco: $elenco <br> 
                            Emissora: $emissora <br> 
                            Episódios: $episodios <br>
                            Classificação: $classif <br> 
                            Foto: $foto <br>
                            Email: $emailDoCliente<br>
                            <a href='http://localhost/TCC_LIVER/cadastro_obra/obra_permitida.php?email=$emailDoCliente&valorobra=$valorobra&nomenovela=$nome&diretornovela=$diretor&anonovela=$ano&sinopsenovela=$sinopse&fotonovela=$foto&elenconovela=$elenco&epsnovela=$episodios&classifnovela=$classif&emissora=$emissora' target='_blank'><button>Permitir</button></a>
